...
def multiprocess_systems(manager, systems):
    ...
    bar_format = (
        ...
        + "S:"
        + manager.term.yellow2("{count_0:{len_total}d}")
        + " "
        + "F:"
        + manager.term.green3("{count_1:{len_total}d}")
        + " "
        + "E:"
        + manager.term.red2("{count_2:{len_total}d}")
        + " "
    ...
    while systems > started or active:
        ...
        if systems > started and len(active) < WORKERS:
            ...
            counter = manager.counter(
                total=files, desc="  System %d:" % started, unit="files", leave=False
            )
            process.start()
            pb_started.update()
        ...
        for system in tuple(active.keys()):
            ...
            if not alive:
                ...
                if process.exitcode != 0:
                    print(
                        "ERROR: Receive exitcode %d while processing System %d"
